...
if not LOCAL:
    app = modal.App("nyc-price-prediction-daily")

    # Create Modal Image and install dependencies
    image = modal.Image.debian_slim().apt_install(["libgomp1"]).pip_install([
        "hopsworks==4.1.5", "seaborn", "joblib", "scikit-learn==1.0.2", "xgboost==1.5", "dataframe-image", "pandas",
        "datetime", "requests", "python-dotenv","numpy==1.26.4"])
